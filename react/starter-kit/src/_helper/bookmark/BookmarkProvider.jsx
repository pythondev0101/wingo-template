...
const BookmarkProvider = (props) => {
  ...
  useEffect(() => {
    const getBookmarkdata = async () => {
      try {
        await axios.get(`http://localhost:8000/bookmark`).then((resp) => {
  ...
  const addNewBookmark = (bookmarkData, img_url) => {
    ...
    const addNewBookmarkData = async () => {
      await axios.post(`http://localhost:8000/bookmark`, bookmarkTemp);
    };
    addNewBookmarkData();
  };
  ...
  const updatebookmark = (id, updatedData, img_url) => {
    ...
    const updatebookmarkData = async () => {
      await axios.put(`http://localhost:8000/bookmark/${id}`, bookmarkTemp);
    };
    updatebookmarkData();
  ...
  const removebookmark = (id) => {
    const removebookmarkData = async () => {
